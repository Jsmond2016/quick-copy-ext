...
环境属于单个接口，不再在 popup 复制选项中维护全局环境状态。复制接口信息时，每条接口独立按以下顺序识别：

1. **响应头 `x-forwarded-for`**
   - 值含 `.fat.` → FAT
   - 值含 `.uat.` → UAT
   - 值含 `.pro.` → PROD
2. **请求 URL**
   - URL 含上述环境标记时使用对应环境
3. **无法识别**
   - 输出 `环境：未识别`，不再默认显示 LOCAL

打开 popup、清空记录、切换 tab 或 domain 都不会影响已经复制的接口环境；每条接口使用自身采集到的响应头和 URL 判断。
