...
function getMenuItems(session: RecordingSession): chrome.contextMenus.CreateProperties[] {
  ...
  if (session.status === 'saving') {
    return [{ ...base, id: `${RECORDING_CONTEXT_MENU_ID}/saving`, title: 'Quick Copy：正在保存录制', enabled: false }];
  }
...
export function registerRecordingContextMenu({
  ...
  chrome.contextMenus.onClicked.addListener((info, tab) => {
    if (!String(info.menuItemId).startsWith(RECORDING_CONTEXT_MENU_ID) || typeof tab?.id !== 'number') {
      return;
    }
