...
/** 根据单个接口的响应头和 URL 识别该接口所属环境。 */
export function getEnvironmentNameFromRequest(
  request: { url: string; headers: Record<string, string> },
): string {
  const matchEnvironmentName = (source: string): string | null => {
    const lower = source.toLowerCase();
    const matched = ENV_MARKERS.find(({ marker }) => lower.includes(marker));
    return matched?.name ?? null;
  };

  const forwardedValues = Object.entries(request.headers ?? {})
    .filter(([key]) => key.trim().toLowerCase() === ENV_HEADER_KEY)
    .map(([, value]) => value);
  for (const forwarded of forwardedValues) {
    const matched = matchEnvironmentName(forwarded);
    if (matched) return matched;
  }

  return matchEnvironmentName(request.url) ?? '未识别';
}
